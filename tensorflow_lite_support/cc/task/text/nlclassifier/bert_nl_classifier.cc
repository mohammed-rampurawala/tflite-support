...
absl::Status BertNLClassifier::Preprocess(
  ...
  PopulateTensor(std::vector<int>(options_.max_seq_len, 0), segment_ids_tensor);
...
StatusOr<std::unique_ptr<BertNLClassifier>>
BertNLClassifier::CreateFromBufferAndOptions(
...
StatusOr<std::unique_ptr<BertNLClassifier>>
BertNLClassifier::CreateFromFdAndOptions(
    int fd, const BertNLClassifierOptions& options,
    std::unique_ptr<tflite::OpResolver> resolver) {
  std::unique_ptr<BertNLClassifier> bert_nl_classifier;
  ASSIGN_OR_RETURN(
      bert_nl_classifier,
