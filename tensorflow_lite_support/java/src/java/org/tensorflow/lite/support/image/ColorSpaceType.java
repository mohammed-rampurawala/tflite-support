...
public enum ColorSpaceType {
   ...
   */
  int getNumElements(int height, int width) {
    // Default for YUV image format. Override the method for other color space types.
    if (this == ColorSpaceType.NV12
        || this == ColorSpaceType.NV21
        || this == ColorSpaceType.YV12
        || this == ColorSpaceType.YV21) {
      return getYUV420NumElements(height, width);
    } else {
      throw new UnsupportedOperationException(
          "getNumElements does not support color space type: " + this.name());
    }
  }
